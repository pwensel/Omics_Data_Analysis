...
#was performed first to avoid determining the most relevant transcript for each unigene based on unnecessary assumptions at the early stage. In addition, given that 
#quantitative asymmetry between up- and downregulated unigenes was strong, SVCD 0.1.0, which does not assume the lack-of-variation between up- and downregulated unigene 
#counts, was used for normalization of unigenes. The mean of raw counts greater than the first quartile (i.e., 5.9 raw counts) as recommended was used during normalization. 
...
#The package DESeq2 provides methods to test for differential expression by use of negative binomial generalized linear models; As input, the DESeq2 package expects 
#raw count data in the form of a matrix of integer values. The DESeq2 model internally corrects for library size, so transformed or normalized values such as counts 
...
# ddsSE <- ddsSE[keep,]
# # Will continue  with counts table as follows:
...
# the reallocation of storage carbon toward lipids played a significant role in conferring the viability of this alga under high salinity stress by remediating high level of 
#cellular stress partially resulted from ROS generated in oxygen-evolving thylakoids as observed in a direct measure of photosystem activities".
